...
def home():
    ...
    # レビュー投稿数が多いユーザーの表示
    # 全期間
    all_review_users = suggestion_doc_ref.document('all').get().to_dict()['most_review_users'][:4]


    # 一週間以内
    oneweek_review_users  = suggestion_doc_ref.document('oneweek').get().to_dict()['most_review_users'][:4]


    # フォロワーが多いユーザーの表示
    most_follow_user = suggestion_doc_ref.document('all').get().to_dict()['most_follow_user'][:4]
